...
class Re_gptMLP(nn.Module):
    def __init__(self, intermediate_size, config):  # in MLP: intermediate_size= 4 * hidden_size
        ...
        self.cca_dropout = nn.Dropout(float(config.dec_ff_dropout))

    def forward(self, hidden_states, is_cca=False):
        hidden_states = self.c_fc(hidden_states)
        hidden_states = self.act(hidden_states)
        hidden_states = self.c_proj(hidden_states)
        if is_cca:
            hidden_states = self.cca_dropout(hidden_states)
        else:
...
class Re_gptBlock(nn.Module):
    ...
    def forward(
        ...
        if exists(self.cross_attn) and exists(retrieval):
            ...
            feed_forward_hidden_states = self.mlp(hidden_states, is_cca=False)
        else:
