...
RE_GPT_PRETRAINED_CONFIG_ARCHIVE_MAP = {
...
class Re_gptConfig(PretrainedConfig):
    ...
    the defaults will yield a similar configuration to that of the Re_gpt
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 50257):
            Vocabulary size of the re_gpt model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`Re_gptModel`]. Vocabulary size of the model. Defines the different
            tokens that can be represented by the *inputs_ids* passed to the forward method of [`Re_gptModel`].
        ...
        type_vocab_size (`int`, *optional*, defaults to 2):
            The vocabulary size of the `token_type_ids` passed when calling [`Re_gptModel`].
    ...
    ```python
    >>> from transformers import Re_gptModel, Re_gptConfig

    >>> # Initializing a Re_gpt  style configuration
    >>> configuration = Re_gptConfig()

    >>> # Initializing a model from the  style configuration
    >>> model = Re_gptModel(configuration)
...
def custom_get_block_length_and_num_blocks(seq_length, window_size):
    """
    Custom implementation for Re_gptAttentionMixin._get_block_length_and_num_blocks to enable the export to ONNX as
...
class Re_gptOnnxConfig(OnnxConfigWithPast):
    @property
